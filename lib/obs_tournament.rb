class ObsTournament
  ...
  has n, :obs_racers, :through => :obs_tournament_participations, :mutable => true

  def racers
    obs_tournament_participations.map(&:obs_racer)
  end
  ...
  def unmatched_racers
    racers - matched_racers - obs_tournament_participations.all(:eliminated => true).obs_racers
  end
  ...
  def never_raced_and_not_eliminated
    ...
    racers - matched - obs_tournament_participations.all(:eliminated => true).obs_racers
  end
